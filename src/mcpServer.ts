...
function compressHover(hover: any): string {
	if (!hover?.contents) return "No hover information found.";
...
async function findFiles(
	...
	for (const file of files) {
		...
		if (file.isDirectory()) {
			await findFiles(filePath, extension, fileList);
...
async function forceIndexWorkspace(client: LspClient, extension: string) {
	try {
		console.error(
			`[Indexer] Performing smart workspace scan for *${extension}...`,
		);
...
server.tool(
	...
	async ({ filePath, line, character, newName }) => {
		try {
			console.error(
				`[Rename] Renaming symbol in ${filePath} at ${line}:${character} to ${newName}`,
			);
...
server.tool(
	...
	"Find a symbol (class, function, etc) by name across the entire project",
	{
		...
		extension: z
			.string()
			.describe("The file extension to search within (e.g. '.ts', '.go')"),
	},
	async ({ query, extension }) => {
		try {
			...
			if (!result || (Array.isArray(result) && result.length === 0)) {
				console.error(
					`[Discovery] LSP returned no results for "${query}". Attempting Smart Fallback...`,
				);
				...
				if (potentialFiles.length > 0) {
					console.error(
						`[Discovery] Found ${potentialFiles.length} potential files via fallback. Forcing LSP indexing...`,
					);
	...
	async ({ filePath, index }) => {
		try {
			...
			if (!actions || index < 1 || index > actions.length) {
				throw new Error(
					"Invalid action index or no actions cached. Run getCodeActions first.",
				);
			}
			...
			if (action.edit) {
				...
				const _result = await client.sendRequest("workspace/executeCommand", {
		...
		try {
			...
			if (webExtensions.includes(ext)) {
				const { stdout } = await execAsync(
					`npx @biomejs/biome check --write "${filePath}"`,
				);
				return {
					content: [
						{
			...
			if (ext === ".go") {
				...
				return {
					content: [
						{ type: "text", text: "[Gofmt] Go file formatted instantly." },
					],
				};
			}
			...
			if (ext === ".rs") {
				...
				return {
					content: [
						{ type: "text", text: "[Rustfmt] Rust file formatted instantly." },
					],
				};
			}
			...
			return {
				content: [
					{
						type: "text",
						text: `Formatting partially failed or found unfixable errors:\n${err.message}`,
					},
				],
				...
				try {
					...
					return {
						content: [
							{
								type: "text",
								text: stdout || "No issues found. The file is clean!",
							},
						],
					};
				} catch (err: any) {
				...
				try {
					...
					return {
						content: [
							{
								type: "text",
								text: stderr || "No issues found. The file is clean!",
							},
						],
					};
				} catch (err: any) {
